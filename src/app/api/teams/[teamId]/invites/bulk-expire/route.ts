...
export async function POST(
	...
	// Check if user has permission to expire invites
	...
	// Update the invites to expired status
	...
	if (updateError) {
		console.error('Error expiring invites:', updateError);
		return NextResponse.json(
			{ error: 'Internal Server Error' },
			{ status: 500 }
		);
	...
	return NextResponse.json(
		{
			message: 'Invites expired successfully',
