...
export async function POST(
	...
	// Check if any invites are already processed
	...
	if (invalidInvites.length > 0) {
		return NextResponse.json(
			{ error: 'Some invites are already processed' },
			{ status: 400 }
		);
	}

	// Update the invites to cancelled status
	...
	if (updateError) {
		console.error('Error cancelling invites:', updateError);
		return NextResponse.json(
			{ error: 'Internal Server Error' },
			{ status: 500 }
		);
